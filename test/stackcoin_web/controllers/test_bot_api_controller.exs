defmodule StackCoinWebTest.BotApiController do
  ...
  describe "POST /api/bot/user/:user_id/send" do
    ...
    test "returns 400 for missing parameters", %{conn: conn, bot_token: bot_token, recipient: recipient} do
      conn =
        conn
        |> put_req_header("authorization", "Bearer #{bot_token}")
        |> post(~p"/api/bot/user/#{recipient.id}/send", %{})
    ...
    test "returns 400 for invalid amount", %{conn: conn, bot_token: bot_token, recipient: recipient} do
      conn =
        conn
        |> put_req_header("authorization", "Bearer #{bot_token}")
        |> post(~p"/api/bot/user/#{recipient.id}/send", %{
